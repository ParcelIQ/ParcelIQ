module Parsers
  class FedexInvoiceParser
    # Maps common report fields to the actual columns in the FedEx invoice
    FIELD_MAPPINGS = {
      # Base transportation
      service_type: :service_name,
      base_charge: :net_charge,

      # Surcharges
      surcharge_type: :surcharge_name,
      surcharge_amount: :charge_amount,

      # Miscellaneous
      misc_type: :misc_description,
      misc_charge: :misc_amount
    }
    ...
    class << self
      ...
      def get_value(entry, field)
        ...
        if entry.respond_to?(column) && entry.has_attribute?(column)
          ...
          Rails.logger.debug("FedEx entry doesn't have column #{column}")
          nil
        end
      end
